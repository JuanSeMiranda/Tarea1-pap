...
public class AltaDeFuncionDeEspectaculo extends JInternalFrame {
	...
	public boolean inicializarComboBoxes() {
		try {
			...
			DefaultComboBoxModel<String> modeloArtistas =    new DefaultComboBoxModel<String>((String[]) this.iControlador.listarArtistas().toArray(new String[this.iControlador.listarArtistas().size()]));
	...
	protected void altaFuncionDeEspectaculoAceptarActionPerformed(ActionEvent arg0) {
		if(checkFormulario()) {
			...
			try 
			{
				this.iControlador.ingresarDatos(nombreEspectaculo, nombreFuncion, fecha,horaInicio,fechaAlta);
	...
	private boolean checkFormulario() {
		if(this.textFieldNomFuncion.getText().isBlank() || this.textFieldFechaDia.getText().isBlank() || this.textFieldFechaMes.getText().isBlank() ||
			this.textFieldFechaAnio.getText().isBlank() || this.textFieldHoraInicio.getText().isBlank() || this.comboBoxPlataforma.getSelectedIndex() == -1 || 
